    ...
    <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 z-50">
      <div className="flex justify-around items-end h-18 px-2 py-2">
        {tabs.map((tab) => {
          ...
          return (
            <Link key={tab.id} href={tab.href} className="flex flex-col items-center w-full">
              {tab.id !== "주문" ? (
                <>
                  <tab.icon className={`h-7 w-7 ${isActive ? "text-primary-custom" : "text-secondary-custom"}`} />
                  <span
                    className={`text-sm font-medium mt-1 ${isActive ? "text-primary-custom" : "text-secondary-custom"}`}
                  >
                    {tab.id}
                  </span>
                </>
                ...
                <>
                  <div className="relative -mt-10">
                    ...
                    >
                      <Image src="/bread-pattern.png" alt="로고" width={36} height={36} className="rounded-full" />
                    </div>
                  </div>
